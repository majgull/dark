...
def task_dirs(spec):
    ...
    A name in two roots is refused, naming both directories: no root
    silently shadows the same name in another."""
    out, owners = [], {}
...
def task_dir_index(spec):
    """{task id: (root, task directory)} for every valid task record across
    the roots of `spec`: the directories that hold a `task.toml`. A name in
    two roots is refused by `task_dirs`, so a name is never shadowed."""
    index = {}
    for root, d in task_dirs(spec):
        if os.path.isfile(os.path.join(d, "task.toml")):
            index[os.path.basename(d)] = (root, d)
    return index
...
def load_tasks(bench_dir, only=None):
    index = task_dir_index(bench_dir)
