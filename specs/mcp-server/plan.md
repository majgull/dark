...
- `TOOLS`, a tuple of six records, one per tool: its name, the dark subcommand it runs, any fixed options (`--no-push` for `dark_run`), its description, and its arguments, each with a JSON Schema type and the CLI option it becomes (`task_dir` becomes `--task-dir`, `judge_tier` becomes `--judge-tier`). `tools/list` is built from this table alone, so the table is the one place a tool is defined.
- `validate(tool, arguments)` returns `None` or one message: a required argument missing, an argument not in the table, or a value of the wrong type (a boolean is not an integer), out of its range, or outside its enum. The caller turns a message into the error `-32602`.
- `build_argv(tool, arguments)` returns the list `["dark", subcommand, *fixed, *options]`. A string or integer becomes one item `--option=value`, so a value that starts with a dash cannot be read by the CLI as another option; a boolean becomes `--option` when true and nothing when false; `files` becomes one item with the paths joined by commas, which is what `dark review --files` takes.
- `handle(message, run)` takes one parsed request and returns the response to write, or `None` for a notification. `serve(stdin, stdout, run=subprocess.run)` reads lines, skips blank ones, parses each, calls `handle`, writes one line per response and flushes, and returns 0 at end of input. `run` is the function that starts the subprocess, so a unit test can hand in a recorder instead.

A tool call starts `build_argv(...)` with `stdin=subprocess.DEVNULL`, standard output captured and decoded as UTF-8 with replacement, standard error inherited, and no timeout. Exit 0 gives `isError` false. Any other exit gives `isError` true and the second content item `exit code N`. `FileNotFoundError` gives `isError` true and the text `dark: command not found on PATH`. `serverInfo.version` is the installed package version from `importlib.metadata`, or `unknown` when dark runs from a checkout that was not installed.
...
Four tools wrap a command that exists: `preflight`, `user`, `long` and `review`. Two do not, and this plan settles both without adding a command the server does not need.

- `dark_run` wraps the existing `dark shift --task-dir <dir> --tier <id> --no-push`, which runs one task through preflight, execution and staging and prints the digest. No new command is added. `--no-push` is always passed so that a remote client cannot cause a digest to be published.
- `dark_ledger_tail` needs `dark ledger-tail [--lines N] [--kind K]`, a new read-only subcommand in `runner/dark/__main__.py`. It reads `host.ledger_path` through `config.load_host` (no token files, no Gitea, no sandbox), prints the last N rows of the ledger that match the kind, oldest first, one JSON object per line, and exits 0 even when nothing matched. It comes first in `tasks.md` because the server is only as useful as the six commands it wraps.
